...
var
  id_f: Integer;
begin
if Application.MessageBox('Вы действительно хотите отвязать данный блок от канала?', 'Подтверждение действия', MB_ICONWARNING + mb_YesNo) = mrYes then
  begin
    if AdvStringGrid1.SelectedRow[0] > 0 then
    begin
      if (ListBox1.ItemIndex > -1) then
...
begin
  ...
  begin
    ...
    if (ListBox1.ItemIndex > -1) then
    begin
     if Application.MessageBox('Вы действительно хотите очистить привязку nooLite на данном блоке?', 'Подтверждение действия', MB_ICONWARNING + mb_YesNo) = mrYes then
  begin
      if (send_enable) then
      begin
        RadioButton1.Checked := true;
  ...
  if step_service_boot = 0 then  begin
    ...
    begin // считывание последнего состояния
      ...
      begin // один нашли
        ...
        begin
          Form1.ComPort1.Port := com_name.Strings[0];
          Form1.ComPort1.BaudRate:=COMbaudrates[strtoint(baudrates.Strings[0])];
          Form1.ComPort1.Open;
          if adapter_name.Strings[0]=DEV_TYPE_0 then
          current_adapter:=0
          else if adapter_name.Strings[0]=DEV_TYPE_9 then
